...
const AccountInformtions: React.FC<ViewsProps> = () => {
  ...
  const [zipCode, setZipCode] = useState(user?.zipCode ?? 0)

  const [openSuccessMessage, setOpenSuccessMessage] = React.useState(false);
  const [openErrorMessage, setOpenErrorMessage] = React.useState(false);

  const [enableInput, setEnableInput] = useState(true);
  const handleToogleInput = () => { setEnableInput(!enableInput) }
        ...
        <Grid className={`${classes.submitContainer}`}>
          {
            enableInput ?
              <Grid item xs={6}>
                <Button variant="outlined" color="primary" type="button" onClick={handleToogleInput}>Mettre votre profil jour</Button>
              </Grid>
              : (
                <>
                  <Grid item xs={6}>
                  ...
                  <Grid item xs={6}>
                    <Button variant="outlined" color="primary" type="button" onClick={handleToogleInput}>Annuler</Button>
                  </Grid>
                </>
              )
